...
def register_movie(movie: MovieCreate) -> MovieResponse:
    ...
    for actor in enriched_data.get("actors", []):
        clean_name = actor.strip()
        get_or_create_person(clean_name)
        create_relationship(clean_name, movie.title, "ACTED_IN")

    director = enriched_data.get("director")
    if director:
        clean_director = director.strip()
        get_or_create_person(clean_director)
        create_relationship(clean_director, movie.title, "DIRECTED")
